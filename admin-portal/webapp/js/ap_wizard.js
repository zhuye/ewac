/* [ ---- Gebo Admin Panel - extended form elements ---- ] */
	...
	ac_ipforms = {
		init: function() {
			$("input.ip4up").bind("change",function(){
				$("input.ip4down").val($("input.ip4up").attr("value"));
			});
			$("input.ip3up").bind("change",function(){
				$("input.ip3down").val($("input.ip3up").attr("value"));
			});
			$("input.ip2up").bind("change",function(){
				$("input.ip2down").val($("input.ip2up").attr("value"));
			});
			$("input.ip2up").bind("change",function(){
				$("input.ip2down").val($("input.ip2up").attr("value"));
			});
	...
	gebo_progressbars = {
		init: function(){
			var iEnd1 = new Date().setTime(new Date().getTime() + 15 * 1000); // now plus 25 secs
			
			$('#progress1').cust_progressbar({
				finish: iEnd1,
				functions: [function() {
					$.sticky("Access Point Found!", {autoclose : 2000, position: "top-right", type: "st-success" });
				},function() {
					$.sticky("IP address configuration complete!", {autoclose : 2000, position: "top-right", type: "st-success" });
				},function() {
					$.sticky("SSID configuration complete!", {autoclose : 2000, position: "top-right", type: "st-success" });
				}]
			});
/*			
			var iEnd1 = new Date().setTime(new Date().getTime() + 15 * 1000); // now plus 25 secs
			$('#progress1').anim_progressbar({
				finish: iEnd1,
				callback: function() {
					$.sticky("IP Config has finished", {autoclose : false, position: "top-right", type: "st-info" });
				}
			});
